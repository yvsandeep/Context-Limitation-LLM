...
def load_llm(llm,load_in_8bit):
    ...
    elif llm == LLM_GO_BRUINS:
        return PdfQA.create_go_bruins(load_in_8bit)
...
def load_emb(emb):
    ...
    if emb == EMB_OPENAI_ADA:
        return PdfQA.create_open_ai_emb()
...
with st.sidebar:
    emb = st.radio("**Select Embedding Model**", [EMB_OPENAI_ADA, EMB_INSTRUCTOR_XL, EMB_SBERT_MPNET_BASE,EMB_SBERT_MINILM],index=1)
    llm = st.radio("**Select LLM Model**", [LLM_OPENAI_GPT35,LLM_FLAN_T5_SMALL,LLM_FLAN_T5_BASE,LLM_FLAN_T5_LARGE],index=2)
